...
def editar_cliente(codigo, nome, cpf, telefone, endereco):
    query = """
    UPDATE clientes
    ...
    try:
        ...
        cursor = cnx.cursor()
        # Executa a transação
        cursor.execute(query, (nome, cpf, telefone, endereco, codigo))
        # Salva (commit) as alterações no banco de dados
        cnx.commit()
    except mysql.connector.Error as e:
        # Se houver erro, desfaz a transação
...
def enviar_clientes(descricao, preco, quantidadeEstoque):
    codigo = random.randint(0, 99999)
    query = """
        INSERT INTO clientes (codigo, descricao, preco, quantidadeEstoque)
        VALUES (%s, %s, %s, %s)
    """
    try:
        # Solicita conexão do pool
